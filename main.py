...
if __name__ == "__main__":
    ...
    # Ant colony
    score = timeit.timeit('ant_colony.run(colony)', setup=f'import ant_colony; colony=ant_colony.create_colony(\'{NETWORK_NAME}\')', number=NUMBER)
    print(score)
    ...
    total_bfs_distance = sum(
        path[0] for node in net.nodes for neighbour in net.nodes if neighbour != node for path in
        bfs_paths_dict[node][neighbour])
